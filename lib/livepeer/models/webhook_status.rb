...
module livepeer
  ...
  class WebhookStatus
    ...
    def self.openapi_types
      {
        :'last_failure' => :'WebhookStatusLastFailure',
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `livepeer::WebhookStatus` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `livepeer::WebhookStatus`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'last_failure')
        self.last_failure = attributes[:'last_failure']
      end
    ...
    def hash
      [last_failure, last_triggered_at].hash
    end
